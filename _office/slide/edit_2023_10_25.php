...
if($_POST['action']=='save'){
	...
	if($img != ""){	
		...
		if($ex[1]=='png'){
			...
			$objThumbImage->createThumb("../../upload/slideVdo/".$ex[0]."_1080.".$ex[1], 1080);
			$objThumbImage->createThumb("../../upload/slideVdo/".$ex[0]."_1600.".$ex[1], 1600);
			$objThumbImage->createThumb("../../upload/slideVdo/".$ex[0]."_1920.".$ex[1], 1920);
		}elseif($ex[1]=='jpg'){
	...
	$query = sqlCommandUpdate($tableIndexSlide,$arrData," ID = '".$_GET['id']."' ");
	$result = $conn->query($query);
	...
	$sql = sqlCommandUpdate($tableIndexSlideDetail,$arrData," `LAG` = '".$_SESSION['lag']."' AND ID_SLIDE = '".$_GET['id']."' ");
...
$tpl->assign("_ROOT.comment",'<span class="badge bg-yellow text-yellow-fg" style="margin-left: 10px;">บันทึกข้อมูลเรียบร้อยแล้ว</span>');
	...
	while($line = $result->fetch_assoc()){
		...
		if(is_file("../../upload/slide/".$line['BANNER_500'])){
			$tpl->assign("img","<img src='../../upload/slide/".$line['BANNER_500']."' height='150'>");
