...
if($_POST['action']=='save'){
	...
	$query = "SELECT MAX(`ORDER`) FROM `$tableIndexSlideCamp` ";
	$result = $conn->query($query);
	...
	$query = sqlCommandInsert($tableIndexSlideCamp,$arrData);
	$result = $conn->query($query);
...
////////////////
	if($newId!=''){
		...
		for ($i=1; $i <= COUNTLANGUAGE('') ; $i++) {
			$arrData['LAG'] = $i; 
			$sql = sqlCommandInsert($tableIndexSlideCampDetail,$arrData);
