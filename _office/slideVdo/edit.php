...
if($_POST['action']=='save'){
	...
	$query = sqlCommandUpdate($tableIndexSlideVdo,$arrData," ID = '".$_GET['id']."' ");
	$result = $conn->query($query);
	...
	$sql = sqlCommandUpdate($tableIndexSlideVdoDetail,$arrData," `LAG` = '".$_SESSION['lag']."' AND ID_SLIDE = '".$_GET['id']."' ");
...
$tpl->assign("_ROOT.comment",'<span class="badge bg-yellow text-yellow-fg" style="margin-left: 10px;">บันทึกข้อมูลเรียบร้อยแล้ว</span>');
	...
	while($line = $result->fetch_assoc()){
		...
		if(is_file("../../upload/slideVdo/".$line['BANNER_500'])){
			$tpl->assign("img","<img src='../../upload/slideVdo/".$line['BANNER_500']."' height='150'>");
