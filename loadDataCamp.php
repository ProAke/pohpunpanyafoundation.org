    ...
    while ($line = $result->fetch_assoc()) {
        ...
        while ($line2 = $result2->fetch_assoc()) {
            ...
            if ($result3->num_rows > '1') {
                array_push($arrayCampModel, '<a href="#" id="district' . $line2['ID_DISTRICT'] . '" data-title="' . $line2['TITLE'] . '" class="drp_province_list  w-dropdown-link" onclick="chooseDistrict(' . $line2['ID_DISTRICT'] . ')" >' . $line2['TITLE'] . '<span style="float: right;">' . $result3->num_rows . ' โรงเรียน</span></a>');
            } else {
