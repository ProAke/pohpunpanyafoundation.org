...
function ReferGroup($type=null,$lag){
global $tpl;
...
//global $lag;
	...
	$query	= "SELECT * FROM `$tablePageDetail` WHERE `PAGE`='$pages' and `LAG`='$lag'";
	$result	= mysql_query($query);
	while($line = mysql_fetch_array($result)) {
		...
		$tpl->assign("keyword",nl2br($line["KEYWORD"]));
		$tpl->assign("title",nl2br($line["TITLE"]));
		$tpl->assign("description",nl2br($line["DESCRIPTION"]));
		$tpl->assign("gotitle",nl2br($line["GOTITLE"]));
		$tpl->assign("gositename",nl2br($line["GOSITENAME"]));
		$tpl->assign("gotype",nl2br($line["GOTYPE"]));

		if(is_file("upload/pages/".$line['GOIMG'])){
			$tpl->assign("ogimg","http://www.tprp.co.th/upload/pages/".$line['GOIMG']."");
		     }
