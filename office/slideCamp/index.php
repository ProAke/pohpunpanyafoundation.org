...
if($_POST['action']=="sort" ){
	for($i=0;$i<count($_POST['order']);$i++){	
		if($_POST['id'][$i]!="" && $_POST['order']!=""){
			...
			$arrData = array();
			$arrData['ORDER'] = $sort;
			$query = sqlCommandUpdate($tableIndexSlideCamp,$arrData,"`ID`='$id'");
			$result = $conn->query($query);
		}
	}
...
if($_GET['action']=='delete' && $_GET['id']!=''){
		$arrData = array();
		$arrData['DEL'] = '1';
		$sql = sqlCommandUpdate($tableIndexSlideCamp,$arrData," `ID` = ".$_GET['id']." ");
...
// echo $query->num_rows;
	...
	while($line = $result->fetch_assoc()){
		...
		$tpl->assign("page",'<img src="../../upload/slideCamp/'.$line['BANNER_500'].'" height="200">');
	}
