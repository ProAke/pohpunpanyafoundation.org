...
$tpl->assignInclude("body", "./template/_tp_partners-ax.html");
$tpl->prepare();
...
while($line = $result->fetch_assoc()){
    ...
    $profileUrl = "./images/".$line['year']."/teachers/popup/".$line['profile_pop'];
...
$iconPack .= '</ul>';
    ...
    $tpl->assign("photo",$url_main."/images/".$line['year']."/teachers/".$role."/".$line['photo']);
    $tpl->assign("profilePOP",$url_main."/images/".$line['year']."/teachers/profile/".$line['profile_pop']);
...
while($line = $result2->fetch_assoc()){
    $tpl->newBlock("COMMITTEE");
    $year = $line['year'];
    $tpl->assign("FullName",$line['fullname']);
    $tpl->assign("FileName",$line['profile_pop']);
    $tpl->assign("photoALN",$url_main."/images/".$year."/teachers/alumni/".$line['photo_aln']);
    $tpl->assign("profilePOP",$url_main."/images/".$year."/teachers/popup/".$line['profile_pop']);  

    }
}else{
    $tpl->newBlock("COMMITTEE_NOT_FOUND"); 
    $tpl->assign("main_url",$url_main);  
}
...
while($line = $result->fetch_assoc()){ 
    ...
    $tpl->newBlock("LECTURER_NOT_FOUND");
    $tpl->assign("main_url",$url_main);  
